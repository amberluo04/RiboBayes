...
pkm.ribo <- Ribo("/Users/weiqinlu/Downloads/all.ribo", rename = rename_default )
...
rc_CDS_w = dcast(rc_CDS[,-5], transcript ~ experiment)
# A value of 500 gives 283 transcripts; 1500 gives 106. 
high_exp = rowSums( cpm(rc_CDS_w[,-c(1, 8:11)]) > 538 ) > 2
sum(high_exp)
...
wavetransform=function(cov){
  experiments=get_experiments(pkm.ribo)
  experiments=experiments[1:6]
  cov2=cov%>%mutate(position=as.numeric(position))%>%arrange(desc(position))
  max=cov2[[1,2]]
  power=floor(log(max, 2))
  bcov=cov%>%arrange(experiment)
  hecc=c()
  noise=c(rep(0.001,nrow(cov)/6))
  noise=jitter(noise, factor = 1, amount=NULL)
  cov=cov%>%mutate(noises=noise)
  for(i in 1:6){
    noisemaker=cov%>%filter(experiment %in% experiments[i])%>%mutate(counts=counts+noises)
    counter=noisemaker$counts
    noisemaker=counter[1:(2^power)]
    noisemaker2=counter[(max-(2^power-1)):max]
    blocks.thr1 <- wavethresh::BAYES.THR(noisemaker, plotfn=FALSE, filter.number=1, 
                                         family = "DaubExPhase", alpha=0, beta=0)
    blocks.thr2 <- wavethresh::BAYES.THR(noisemaker2, plotfn=FALSE, filter.number=1, 
                                         family = "DaubExPhase", alpha=0, beta=0)
...
wavelettransform=function(cov, threshold){
  ...
  acov=cov%>%mutate(position=as.numeric(position))%>%group_by(experiment)%>%
    mutate(mean=(counts-mean(counts))/(sd(counts)))
...
wavepeakstats=function(transcript, threshold){
  ...
  kappaset=kappaset%>%mutate(transcript=transcript)
  kappaset=kappaset%>%mutate(position=paste(position, transcript))%>%select(-transcript)
  ahh=diffpositions
  ...
  diffkappaset=diffkappaset%>%mutate(transcript=transcript)
  diffkappaset=diffkappaset%>%mutate(position=paste(position, transcript))%>%select(-transcript)
  allpos=kappaset$position
  numpeaks=c(nrow(set), nrow(diffkappaset))
  newlist=list(kappaset, diffkappaset, numpeaks, allpos, ahh)
  return(newlist)
}
...
wavepeakstats=function(transcript, threshold){
  ...
  ah=differentialpeaks(set)
  allpeaks=ah[[1]]
  allpeaks=allpeaks%>%mutate(transcript=transcript)%>%mutate(position2=paste(position, transcript))%>%select(position2, statistic, p)
  allpeaks=allpeaks[,c(1, 3, 2)]
  diffpeaks=ah[[2]]
  diffpositions=diffpeaks[,c(1:3)]
  set=set%>%mutate(sig=0)
  for(i in 1:nrow(set)){
  ...
  allpos=kappaset$position
  numpeaks=c(nrow(set), nrow(diffkappaset))
  return(list(allpeaks, diffpeaks))
}
...
threshold=9
wavepeakcounts9=mcmapply(function(x,y){
